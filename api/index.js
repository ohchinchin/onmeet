...
app.post('/chat', async (req, res) => {
    ...
    if (agent.isModerator) {
        systemPrompt = isSummaryTurn 
            ? `あなたは司会者の${agent.name}です。議論の要約と最終的な総括を行ってください。`
            : `あなたは司会者の${agent.name}です。議論が盛り上がるようにリードしてください。`;
    } else {
        systemPrompt = `あなたは${agent.name}（${agent.role}）です。性格: ${agent.personality}。自分の専門的な立場から意見を述べてください。`;
    }
